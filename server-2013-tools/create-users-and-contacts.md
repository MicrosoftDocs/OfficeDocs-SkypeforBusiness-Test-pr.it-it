...
ms:mtpsurl: https://technet.microsoft.com/it-it/library/JJ945587(v=OCS.15)
ms:contentKeyID: 52062491
ms.date: 08/02/2014
...
Here is a list of terms and definitions that you might find useful as you read through this topic.
  ...
  - Organizational Unit – The Servizi di dominio Active Directory organizational unit (OU).
...
8.  Select the **Voice Enabled** check box if you want to enable the test users for VoIP aziendale.
...
When you click the Create Distribution Lists button, the tool queries Servizi di dominio Active Directory to see if distribution lists matching the prefix and numbers already exist. The tool will create only the ones that do not already exist. When adding members to these newly created Distribution Lists, it will pick the users from the range specified on the User Creation tab.
